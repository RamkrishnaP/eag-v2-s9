...
class MemoryItem(BaseModel):
    ...
    user_query: Optional[str] = None  # 🆕 Added for history tracking
...
class MemoryManager:
    ...
    def __init__(self, session_id: str, memory_dir: str = "memory"):
        ...
        parts = session_part.replace("session-", "").split(
            "-"
        )  # ["1764356085", "d89952"]
        timestamp_part = parts[0]
        uid_part = parts[1] if len(parts) > 1 else ""

        # Create path: memory/2025/11/29/session/{timestamp}/{uid}/session-2025-11-29-session-{timestamp}-{uid}.json
